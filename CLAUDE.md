...
> 本项目从 NyaaFrame 模板衍生，适用"**部分适用**"级别——已有 SSOT（`BLUEPRINT.md`），跳过初始设计和审核阶段，从当前里程碑 P 阶段续接。

### 版本与阶段（V + P）

- **V（闭环版本）**：一个对外可用、功能闭环的版本。
- **P（功能模块阶段）**：对应 `BLUEPRINT.md` 中里程碑（M）下的子任务拆分，每个 P 必须**可独立验证、可独立提交**。
...
1. **同步蓝图** — 通过 `sync-blueprint` skill 更新 `BLUEPRINT.md` 状态标记
2. **Git 提交与推送** — 通过 `commit-push` skill，提交前必须做 `git status` 和 secret 检查
3. **更新或创建交接文档** — 落于 `.docs/阶段交接-XXX.md`，包含以下章节：
   - 交接目的 + 必读文档列表（至少含 `BLUEPRINT.md`）
   - 当前进度（本 P 完成了什么）
   - 本轮已修复/已实现（按文件列出）
   - 仍需验证/已知问题
   - **续接提示词**（可直接粘贴给新对话的提示词，约 10-20 行，含必读文档、当前进度、下一步行动、关键约束和 LLM 对话框架约束）
4. **更新 Memory** — 关键节点写入 memory 跟踪

### 跨对话接续

新对话继续开发时：读取本 CLAUDE.md → 读取 `BLUEPRINT.md` → 读取最新交接文档 → 根据"续接提示词"确定下一步 → plan 模式进入。
